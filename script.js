const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwDUqfNKS7K_czrTeU9OIih22ADZ5mJ0yblqsQTTWcdqYb7h43nDT2cWQP0HfZt4uqNpQ/exec';
...
const appState = {
    ...
    submitEvaluation() {
        ...
        // Preparando dados para o Google Sheets
        const formData = new FormData();
        formData.append('Nivel', this.selectedLevel);
        formData.append('Sala', this.selectedClass);
        formData.append('Estrelas', this.starsRating);
        ...
        fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST',
            body: formData,
            mode: 'no-cors' // Crucial para funcionar sem servidor próprio
        }).then(() => {
            navigation.showScreen('screen-success', 'Obrigado!');
            btnSubmit.innerText = "Enviar Avaliação";
            btnSubmit.disabled = false;
        }).catch(error => {
            alert("Houve um erro na comunicação. Verifique sua conexão e tente novamente.");
            btnSubmit.innerText = "Enviar Avaliação";
            btnSubmit.disabled = false;
        });
    }
